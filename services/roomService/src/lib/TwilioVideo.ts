...
const MAX_ALLOWED_SESSION_DURATION = 14400; //4 hours: each client will time out after 4 hours of video and need to refresh
...
export default class TwilioVideo implements IVideoClient {
    ...
    public static getInstance(): TwilioVideo {
        if (!TwilioVideo._instance) {
            ...
            TwilioVideo._instance = new TwilioVideo(process.env.TWILIO_ACCOUNT_SID, process.env.TWILIO_API_AUTH_TOKEN,
