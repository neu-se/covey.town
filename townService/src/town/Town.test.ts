...
describe('Town', () => {
  ...
  describe('addPlayer', () => {
    ...
    describe('[T1] interactableUpdate callback', () => {
      ...
      it('Should not throw an error for any interactable area that is not a viewing area', () => {
        expect(() =>
          interactableUpdateHandler({ id: 'Name1', topic: nanoid(), occupantsByID: [] } as any as ViewingAreaModel),
        ).not.toThrowError();
      });
  ...
  describe('Socket event listeners created in addPlayer', () => {
    describe('on socket disconnect', () => {
      ...
      it('Removes the player from any active conversation area', () => {
        ...
        expect(
          town.addConversationArea({ id: 'Name1', topic: 'test', occupants: [], type: 'ConversationArea' }),
        ).toBeTruthy();
      ...
      it('Removes the player from any active viewing area', () => {
        ...
        expect(
          town.addViewingArea({ id: 'Name3', isPlaying: true, elapsedTimeSec: 0, video: nanoid(), occupants: [], type: 'ViewingArea' }),
        ).toBeTruthy();
  ...
  describe('addConversationArea', () => {
    ...
    it('Should return false if no area exists with that ID', () => {
      expect(
        town.addConversationArea({ id: nanoid(), topic: nanoid(), occupants: [], type: 'ConversationArea' }),
      ).toEqual(false);
    });
    it('Should return false if the requested topic is empty', () => {
      expect(town.addConversationArea({ id: 'Name1', topic: '', occupants: [], type: 'ConversationArea' })).toEqual(
        false,
      );
      expect(
        town.addConversationArea({ id: 'Name1', topic: undefined, occupants: [], type: 'ConversationArea' }),
      ).toEqual(false);
    });
    it('Should return false if the area already has a topic', () => {
      expect(
        town.addConversationArea({ id: 'Name1', topic: 'new topic', occupants: [], type: 'ConversationArea' }),
      ).toEqual(true);
      expect(
        town.addConversationArea({ id: 'Name1', topic: 'new new topic', occupants: [], type: 'ConversationArea' }),
      ).toEqual(false);
    });
    describe('When successful', () => {
      ...
      beforeEach(() => {
        playerTestData.moveTo(45, 122); // Inside of "Name1" area
        expect(
          town.addConversationArea({ id: 'Name1', topic: newTopic, occupants: [], type: 'ConversationArea' }),
        ).toEqual(true);
      });
  ...
  describe('[T1] addViewingArea', () => {
    ...
    it('Should return false if no area exists with that ID', () => {
      expect(
        town.addViewingArea({ id: nanoid(), isPlaying: false, elapsedTimeSec: 0, video: nanoid(), occupants: [], type: 'ViewingArea' }),
      ).toBe(false);
    });
    it('Should return false if the requested video is empty', () => {
      expect(
        town.addViewingArea({ id: 'Name3', isPlaying: false, elapsedTimeSec: 0, video: '', occupants: [], type: 'ViewingArea'  }),
      ).toBe(false);
      expect(
        town.addViewingArea({ id: 'Name3', isPlaying: false, elapsedTimeSec: 0, video: undefined, occupants: [], type: 'ViewingArea'  }),
      ).toBe(false);
    });
    it('Should return false if the area is already active', () => {
      expect(
        town.addViewingArea({ id: 'Name3', isPlaying: false, elapsedTimeSec: 0, video: 'test' , occupants: [], type: 'ViewingArea'}),
      ).toBe(true);
      expect(
        town.addViewingArea({ id: 'Name3', isPlaying: false, elapsedTimeSec: 0, video: 'test2', occupants: [], type: 'ViewingArea' }),
      ).toBe(false);
    });
    describe('When successful', () => {
  ...
  describe('initializeFromMap', () => {
    ...
    describe('Updating interactable state in playerMovements', () => {
      beforeEach(async () => {
        ...
        expect(town.addConversationArea({ id: 'Name1', topic: 'test', occupants: [], type: 'ViewingArea' })).toBe(
          true,
        );
      });
