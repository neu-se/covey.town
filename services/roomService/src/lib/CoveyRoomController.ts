...
export default class CoveyRoomController {
    ...
    async addPlayer(newPlayer: Player): Promise<PlayerSession> {
        const new_session = new PlayerSession(newPlayer);

        this.sessions.push(new_session);
        this.players.push(newPlayer);

        //Create a video token for this user to join this room
        new_session.videoToken = await this.videoClient.getTokenForRoom("demoRoom", newPlayer.id);
     ...
     */
    destroySession(session: PlayerSession) {
     ...
     */
    updatePlayerLocation(player: Player, location: UserLocation) {
     ...
     */
    addRoomListener(listener: CoveyRoomListener) {
     ...
     */
    removeRoomListener(listener: CoveyRoomListener) {
